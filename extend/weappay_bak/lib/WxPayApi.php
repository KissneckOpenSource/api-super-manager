...
//use app\api\exception\weappay\lib\WxPayResults;
//use app\api\exception\weappay\lib\WxPayNotifyReply;
//
//use app\api\exception\weappay\lib\WxPayReport;
//use think\Db;
...
class WxPayApi
{
    ...
    public static function unifiedOrder($inputObj, $timeOut = 6, $appid = '', $mchid = '', $paysecret = '')
    {
        ...
        //签名
        $inputObj->SetSign($paysecret);
        $xml = $inputObj->ToXml();

        $startTimeStamp = self::getMillisecond(); //请求开始时间
        $response = self::postXmlCurl($xml, $url, false, $timeOut);
        $result = WxPayResults::Init($response, $paysecret);
     ...
     * @return 成功时返回，其他抛异常
     */
    public static function refund($inputObj, $timeOut = 6,$sign = "",$nonce_str="")
    {
    ...
    public static function notify($callback, &$msg)
    {
        ...
        $xmlArg    = json_decode(json_encode(simplexml_load_string($xml, 'SimpleXMLElement', LIBXML_NOCDATA)), true);
        $order_oid = Db::name('order')->where('id', $xmlArg['attach'])->value('oid');
        $paysecret = WxPayConfig::get_wx_config_info('paysecret', $order_oid);  //支付密钥
        //Log::DEBUG("微信异步支付反馈调试:" . json_encode($xmlArg));

        //如果返回成功则验证签名
        try {
            $result = WxPayResults::Init($xml, $paysecret);
     ...
     * @return static
     */
    public static function postXmlCurl($xml, $url, $useCert = false, $second = 30, $oid = 0)
    {
        $ch = curl_init();
        //设置超时
     ...
     * 微信提现签名
     */
    public static function toMakeSign($data, $appsercet)
    {
        //获取微信支付秘钥
        $key = $appsercet;
        // 去空
     ...
     * @throws \app\api\exception\wepay\lib\WxPayException
     */
