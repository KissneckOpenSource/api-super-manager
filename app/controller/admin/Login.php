...
class Login extends Base {
    ...
    public function getAccessMenuData(int $uid): array {
        ...
        if ($isSupper) {
            ...
            printLog("权限授权获取超管：",$access,"access");
            ...
            if (isset($groups) && $groups->group_id) {
                ...
                printLog("权限授权获取1：",$access,"access");
                $menus = (new AdminMenu())->whereIn('url', $access)->where('show', 1)->where('router', '<>', '')->select();
                printLog("权限授权获取2：",$menus,"access");
                $returnData = Tools::listToTree(Tools::buildArrFromObj($menus));
                printLog("权限授权获取3：",$returnData,"access");
                //RouterTool::buildVueRouter($returnData);
            }
        }
        printLog("权限授权获取4：",$returnData,"access");
